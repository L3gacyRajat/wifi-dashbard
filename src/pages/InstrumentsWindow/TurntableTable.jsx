...
const TurntableTable = () => {
  const [data, setData] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      readRemoteFile('src/assets/Turntable_status.csv', {
        complete: (parsed) => {
  ...
  const columns = [
    { Header: 'Turntable Type', accessor: 'turntableType' },
    { Header: 'Turntable Name', accessor: 'turntableName' },
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <button
          ...
          onClick={() => alert(`Reload ${row.original.turntableName}`)}
        >
          Reload
        </button>
      ),
